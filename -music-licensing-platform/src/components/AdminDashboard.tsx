...
export function AdminDashboard({ currentUserId }: AdminDashboardProps) {
  ...
  // Multi-select & Bulk Copyright Labels
  const [selectedTrackIds, setSelectedTrackIds] = useState<string[]>([]);
  const [bulkCopyrightLabel, setBulkCopyrightLabel] = useState("");
  const [bulkLicenseTerm, setBulkLicenseTerm] = useState("");
  const [isBulkLabelUpdating, setIsBulkLabelUpdating] = useState(false);
  ...
  const loadStats = async () => {
    try {
      const res = await fetch(getApiUrl(`/api/stats/summary`), {
        headers: { 'Authorization': `x-user-id ${currentUserId}` }
      });
      const data = await res.json();
  ...
  const loadTracks = async () => {
    try {
      const res = await fetch(getApiUrl(`/api/tracks`), { headers: { 'Authorization': `x-user-id ${currentUserId}` } });
      if (res.ok) {
        const data = await res.json();
        setTracksList(data);
      }
  ...
  const loadPlaylists = async () => {
    try {
      const res = await fetch(getApiUrl(`/api/playlists`), { headers: { 'Authorization': `x-user-id ${currentUserId}` } });
      if (res.ok) {
        const data = await res.json();
  ...
  const loadInvoices = async () => {
    try {
      const res = await fetch(getApiUrl(`/api/invoices`), { headers: { 'Authorization': `x-user-id ${currentUserId}` } });
      if (res.ok) {
        const data = await res.json();
  ...
  const loadUsers = async () => {
    try {
      const res = await fetch(getApiUrl(`/api/users`), { headers: { 'Authorization': `x-user-id ${currentUserId}` } });
      if (res.ok) {
        const data = await res.json();
        setUsersList(data);
      }
  ...
  const loadTokens = async () => {
    try {
      const res = await fetch(getApiUrl(`/api/tokens/active`), { headers: { 'Authorization': `x-user-id ${currentUserId}` } });
      if (res.ok) {
        const data = await res.json();
  ...
  const loadPages = async () => {
    try {
      const res = await fetch(getApiUrl(`/api/access-pages`), { headers: { 'Authorization': `x-user-id ${currentUserId}` } });
      if (res.ok) {
        const data = await res.json();
  ...
  const loadSettings = async () => {
    try {
      const res = await fetch(getApiUrl(`/api/settings`), { headers: { 'Authorization': `x-user-id ${currentUserId}` }});
      if (res.ok) {
  ...
  const handleSaveSettings = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(getApiUrl(`/api/settings`), {
        method: "PUT",
        headers: { "Content-Type": "application/json", "Authorization": `x-user-id ${currentUserId}` },
    ...
    try {
      const res = await fetch(getApiUrl(url), {
        ...
        headers: { 
          ...
          'Authorization': `x-user-id ${currentUserId}`
        },
  ...
  const handleBulkImportTracks = async () => {
    ...
    try {
      if (!bulkImportJson.trim()) {
        throw new Error("Wklej zawartość JSON przed rozpoczęciem importu.");
      }

      let parsedData;
      try {
        parsedData = JSON.parse(bulkImportJson);
      } catch (e: any) {
        throw new Error(`Niepoprawny format kodu JSON: ${e.message}`);
      }

      if (!Array.isArray(parsedData)) {
        throw new Error("Wklejony dokument JSON musi posiadać strukturę tablicy obiektów: [ { ... }, { ... } ]");
      }

      const res = await fetch(getApiUrl("/api/tracks/bulk"), {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `x-user-id ${currentUserId}`
      ...
      if (!res.ok) {
        throw new Error(responseData.error || "Serwer odrzucił paczkę danych.");
      }

      setBulkImportSuccess(`Zatwierdzono stan! Pomyślnie wdrożono ${responseData.count} utworów do bazy hrl_track.`);
      addLog(`Masowy import B2B: Utworzono ${responseData.count} utworów.`);
      setBulkImportJson("");
      
      loadTracks();
      loadStats();
    } catch (err: any) {
      setBulkImportError(err.message || "Błąd podczas wdrażania paczki danych.");
    } finally {
      setIsImporting(false);
    }
  };

  const handleBulkUpdateLabels = async () => {
    setBulkLabelError(null);
    setBulkLabelSuccess(null);
    setIsBulkLabelUpdating(true);

    try {
      if (selectedTrackIds.length === 0) {
        throw new Error("Wybierz przynajmniej jeden utwór z listy.");
      }
      if (!bulkCopyrightLabel.trim() && !bulkLicenseTerm.trim()) {
        throw new Error("Wpisz oznaczenie praw autorskich lub warunki licencji przed aktualizacją.");
      }

      const res = await fetch(getApiUrl("/api/tracks/bulk-update-labels"), {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `x-user-id ${currentUserId}`
        ...
        body: JSON.stringify({
          ...
          copyrightLabel: bulkCopyrightLabel.trim() || undefined,
          licenseTerms: bulkLicenseTerm.trim() || undefined
        })
      });

      const data = await res.json();
      if (!res.ok) {
        throw new Error(data.error || "Wystąpił błąd serwera podczas masowej aktualizacji.");
      }

      setBulkLabelSuccess(`Zaktualizowano pomyślnie ${data.count} utworów.`);
      addLog(`Masowa edycja licencji: Zaktualizowano ${data.count} utworów.`);
      setSelectedTrackIds([]); // Clear selection upon success
      setBulkCopyrightLabel("");
      setBulkLicenseTerm("");
      
      loadTracks();
    } catch (err: any) {
      setBulkLabelError(err.message || "Błąd podczas masowej modyfikacji.");
    } finally {
      setIsBulkLabelUpdating(false);
    }
  };
              ...
              {activeTab === 'tracks' && (
                <div className="space-y-6 animate-fade-in">
                  <div className="flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-2.5">
                      <button 
                        ...
                        className="flex items-center gap-1.5 bg-orange-600 hover:bg-orange-700 text-xs font-bold text-white px-4 py-2 rounded-xl transition cursor-pointer"
                      >
                        <Database className="h-4 w-4" />
                        <span>Masowy Import JSON</span>
                      </button>
                      <button 
                        onClick={() => {
                          setEditingTrack(null);
                  ...
                  <div className="bg-zinc-900/40 border border-zinc-900 p-4 rounded-2xl flex flex-col sm:flex-row gap-4 items-center justify-between shadow-md">
                    ...
                    <div className="flex flex-wrap items-center gap-3.5 self-end sm:self-auto select-none">
                      {filteredTracks.length > 0 && (
                        <button
                          onClick={() => {
                            const allFilteredIds = filteredTracks.map((t) => t.id);
                            const allSelected = allFilteredIds.every((id) => selectedTrackIds.includes(id));
                            if (allSelected) {
                              setSelectedTrackIds((prev) => prev.filter((id) => !allFilteredIds.includes(id)));
                            } else {
                          ...
                          className="text-[10px] font-black uppercase font-mono tracking-wider bg-zinc-950 hover:bg-zinc-900 border border-zinc-900 text-zinc-400 hover:text-white px-3.5 py-2 rounded-xl transition cursor-pointer flex items-center gap-1.5"
                        >
                          <Check className="h-3.5 w-3.5 text-orange-500" />
                          <span>
                            {filteredTracks.map((t) => t.id).every((id) => selectedTrackIds.includes(id)) 
                              ? "Odznacz wszystkie" 
                              : "Zaznacz wszystkie"}
                          </span>
                        </button>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredTracks.map((track) => {
                      ...
                      return (
                        <div 
                          key={track.id} 
                          className={`bg-[#121318] border rounded-2xl overflow-hidden flex flex-col justify-between transition-all duration-250 ${
                            isSelected 
                              ? 'border-orange-500/60 shadow-[0_0_15px_rgba(249,115,22,0.1)] bg-orange-500/[0.02]' 
                        ...
                        >
                            ...
                            <div className="pt-1 select-none">
                              ...
                              >
                                {isSelected ? (
                                  ...
                                  <div className="w-1.5 h-1.5 rounded bg-zinc-700 opacity-0 hover:opacity-100 transition" />
                                )}
                              </button>
                            </div>
                          ...
                          <div className="bg-[#050508] px-4 py-3 border-t border-gray-900/60 text-[10px] font-mono space-y-2">
                            ...
                            {/* Oznaczenia Prawnicze / Licencyjne */}
                            {(track.copyrightLabel || track.licenseTerms) && (
                              <div className="border-t border-zinc-950 pt-2 space-y-1 bg-black/10 -mx-4 px-4 pb-1 rounded-b">
                                {track.copyrightLabel && (
                                  <div className="flex items-center gap-1 text-[8.5px] text-emerald-400 font-sans font-bold uppercase tracking-wide">
                                    <span className="font-mono text-[7px] bg-emerald-500/10 border border-emerald-500/20 px-1 py-0.2 rounded font-black text-emerald-400">OZZ EXEMPT</span>
                                    <span className="truncate max-w-[160px] text-[#f1f5f9] select-all lowercase font-mono font-medium">{track.copyrightLabel}</span>
                                  </div>
                                )}
                                {track.licenseTerms && (
                                  <div className="flex items-center gap-1 text-[8.5px] text-blue-400 font-sans font-bold uppercase tracking-wide">
                                    <span className="font-mono text-[7px] bg-blue-500/10 border border-blue-500/20 px-1 py-0.2 rounded font-black text-blue-400">LICENCJA</span>
                                    <span className="truncate max-w-[170px] text-[#f1f5f9] select-all font-mono font-medium">{track.licenseTerms}</span>
                                  </div>
                                )}
                              </div>
                            )}
                  ...
                  {selectedTrackIds.length > 0 && (
                    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 w-full max-w-5xl px-4">
                      <div className="bg-[#121318]/95 backdrop-blur-md border border-orange-500/40 rounded-3xl p-4.5 shadow-[0_12px_40px_rgba(249,115,22,0.2)] flex flex-col lg:flex-row items-center justify-between gap-4 transition-all duration-300 animate-slide-up">
                        
                        {/* Selected info with clear action */}
                        <div className="flex items-center gap-3.5 self-start lg:self-center">
                          <div className="bg-orange-500/10 text-orange-400 font-mono font-black border border-orange-500/25 px-3.5 py-2 rounded-2xl text-[11px] flex items-center gap-2.5 shadow-inner">
                            <span className="tracking-wider uppercase text-[10px] opacity-85">ZAZNACZONO:</span>
                            <span className="bg-orange-500 text-black px-2 py-0.5 rounded-xl font-bold text-xs leading-none">
                          ...
                          <button
                            ...
                            className="text-[10px] font-black text-zinc-500 hover:text-white uppercase tracking-wider font-mono cursor-pointer transition border border-zinc-900 hover:border-zinc-800 px-3 py-2 rounded-xl bg-black/40"
                          >
                            Wyzeruj [X]
                          </button>
                        </div>
                        ...
                        {/* Presets and Inputs */}
                        <div className="flex flex-wrap items-center gap-4 w-full lg:w-auto flex-1 lg:justify-end">
                          <div className="space-y-1.5 flex-1 min-w-[160px]">
                            <label className="text-[9px] font-black uppercase tracking-widest text-[#94a3b8] block">Preset / Prawa Autorskie (OZZ)</label>
                            <div className="flex gap-2">
                              <select
                                value={bulkCopyrightLabel}
                                onChange={(e) => setBulkCopyrightLabel(e.target.value)}
                                className="bg-black text-[10.5px] font-mono text-zinc-300 border border-zinc-900 rounded-xl px-2.5 py-2 max-w-[140px] focus:outline-none focus:border-orange-500/50"
                              >
                                <option value="">-- Wybierz --</option>
                                <option value="exempt-drm-hrl-direct">exempt-drm-hrl-direct</option>
                                <option value="copyright-free-public-domain">copyright-free-public-domain</option>
                                <option value="100%-exempt-oz-no-zaiks">100%-exempt-oz-no-zaiks</option>
                                <option value="hrl-direct-exclusive-catalog">hrl-direct-exclusive-catalog</option>
                              </select>
                              <input
                                type="text"
                                ...
                                value={bulkCopyrightLabel}
                                onChange={(e) => setBulkCopyrightLabel(e.target.value)}
                                className="bg-zinc-950 text-white placeholder-zinc-700 font-mono text-[10.5px] px-3 py-2 rounded-xl border border-zinc-900 focus:border-orange-500/40 focus:outline-none flex-1 min-w-[110px]"
                              />
                            </div>
                          </div>

                          <div className="space-y-1.5 flex-1 min-w-[160px]">
                            <label className="text-[9px] font-black uppercase tracking-widest text-[#94a3b8] block">Warunki Licencyjne / Term</label>
                            <div className="flex gap-2">
                              <select
                                value={bulkLicenseTerm}
                                onChange={(e) => setBulkLicenseTerm(e.target.value)}
                                className="bg-black text-[10.5px] font-mono text-zinc-300 border border-zinc-900 rounded-xl px-2.5 py-2 max-w-[140px] focus:outline-none focus:border-orange-500/50"
                              >
                                <option value="">-- Wybierz --</option>
                                <option value="HRL-direct-2026-unlimited">HRL-direct-2026-unlimited</option>
                                <option value="B2B Commercial Royalty-Free">B2B Commercial Royalty-Free</option>
                                <option value="Creative Commons Zero (CC0)">Creative Commons Zero (CC0)</option>
                                <option value="Exempt-OZZ-B2B-Standard">Exempt-OZZ-B2B-Standard</option>
                              </select>
                              <input
                                type="text"
                                placeholder="Lub wpisz własną..."
                          ...
                          {/* Submit Action */}
                          <div className="pt-3.5 lg:pt-0 self-end">
                            <button
                              onClick={handleBulkUpdateLabels}
                              disabled={isBulkLabelUpdating || (!bulkCopyrightLabel.trim() && !bulkLicenseTerm.trim())}
                              className={`bg-orange-600 hover:bg-orange-700 text-xs font-bold text-white px-5.5 py-2.5 rounded-xl transition cursor-pointer flex items-center gap-2 select-none active:scale-95 duration-150 ${
                                (isBulkLabelUpdating || (!bulkCopyrightLabel.trim() && !bulkLicenseTerm.trim())) ? 'opacity-50 cursor-not-allowed' : ''
                              }`}
                            >
                              {isBulkLabelUpdating ? (
                                <>
                                  <span className="w-3.5 h-3.5 border-2 border-white/20 border-t-white rounded-full animate-spin" />
                                  <span>Trwa edycja...</span>
                                </>
                              ) : (
                                <>
                                  <Sliders className="h-4 w-4 text-white" />
                                  <span>Zastosuj masowo</span>
                                </>
                              )}
                            </button>
                          </div>
                      ...
                      {/* Status feedback boxes appended cleanly underneath/above bar */}
                      {(bulkLabelError || bulkLabelSuccess) && (
                        <div className="mt-2.5 max-w-xl mx-auto animate-fade-in text-[10px] text-center select-text font-semibold font-mono">
                          {bulkLabelError && (
                            <div className="bg-red-500/10 border border-red-500/30 text-red-400 py-1.5 px-3.5 rounded-xl">
                              ⚠️ {bulkLabelError}
                            </div>
                          )}
                          {bulkLabelSuccess && (
                            <div className="bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 py-1.5 px-3.5 rounded-xl">
                              ✨ {bulkLabelSuccess}
                            </div>
                          )}
                        </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="bg-[#121318] border border-gray-900 rounded-2xl p-5 space-y-4">
                      ...
                      <p className="text-[11px] text-gray-400 leading-normal font-sans">
                        Używaj autoryzacji nagłówka <code className="bg-black text-orange-400 px-1 rounded font-mono">Authorization: x-user-id</code> lub stałego tokenu integracyjnego w celach pobierania list playlist i wykazów utworów.
                      </p>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="space-y-1">
                          ...
                          <pre className="bg-black text-red-400 text-[9.5px] p-2.5 rounded-xl border border-gray-950 overflow-x-auto leading-relaxed select-all">
                            curl -X GET "https://hrl-music.com/api/my-playlists" \<br />
                            &nbsp;&nbsp;-H "Authorization: x-user-id administrator" \<br />
...
add_action( 'template_redirect', function() {
    if ( ! is_oauth_or_api_call() ) {
      ...
      {/* MODAL MASOWEGO IMPORTU UTWORÓW */}
      {showBulkImportModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm animate-fade-in">
          <div className="bg-[#121318] border border-gray-800 rounded-3xl p-6 w-full max-w-2xl max-h-[92vh] overflow-y-auto space-y-5 shadow-2xl text-left">
            <div className="flex justify-between items-center border-b border-gray-900 pb-3">
              <div className="flex items-center gap-2">
                <Database className="h-5 w-5 text-orange-500" />
                <h3 className="text-base font-extrabold text-white uppercase tracking-wider">
                  Masowy Import Specyfikacji JSON
                </h3>
              </div>
              <button 
            ...
            <p className="text-xs text-gray-400 leading-relaxed">
              Zarządzaj i filtruj całe biblioteki szybko i bezpiecznie. Wklej poprawną tablicę JSON z definicjami nowych utworów. System automatycznie przefiltruje struktury, wygeneruje brakujące identyfikatory cyfrowe, przypisze losowe numery ISRC oraz wdroży dane do bazy <span className="text-orange-400 font-mono font-bold">hrl_track</span>.
            </p>

            <div className="space-y-4">
              <div className="bg-black/60 rounded-xl p-3 border border-zinc-900 border-gray-950/80 text-[10px] font-mono leading-relaxed text-zinc-400">
                <div className="flex justify-between items-center mb-1.5 pb-1 border-b border-gray-900">
                  <span className="text-gray-500 uppercase tracking-widest font-black text-[9px]">Sugerowany Schemat/Paczka JSON</span>
                  <button 
                    onClick={() => {
                      const sample = [
                        {
                          "title": "Acoustic Lounge Wave",
                          "artist": "Amber Grid",
                          "album": "Nordic Warmth",
                          "genre": "pop",
                          "bpm": 105,
                        ...
                        {
                          "title": "Midnight Espresso lofi",
                          "artist": "HRL Jazz Beats",
                    ...
                    className="text-[9px] font-bold bg-orange-600/20 hover:bg-orange-650/30 border border-orange-500/20 px-2.5 py-1 rounded text-orange-400 uppercase transition"
                  >
                    Wklej Szablon Testowy
                  </button>
                </div>
                <div className="max-h-36 overflow-y-auto whitespace-pre font-mono text-zinc-500 select-all">
                  {"[\n  {\n    \"title\": \"string (wymagane)\",\n    \"artist\": \"string (wymagane)\",\n    \"album\": \"string (opcjonalne)\",\n    \"genre\": \"string (opcjonalne, domyślnie 'jazz')\",\n    \"bpm\": \"number (opcjonalne)\",\n    \"duration\": \"number (sekundy, opcjonalne)\",\n    \"explicit\": \"boolean (opcjonalne)\",\n    \"timeOfDay\": \"string[] ('morning'|'afternoon'|'evening'|'night')\",\n    \"mood\": \"string[]\",\n    \"isrc\": \"string (autogenerowany w razie braku)\",\n    \"cover\": \"string (opcjonalny URL okładki)\"\n  }\n]"}
                </div>
              </div>

              <div className="space-y-1.5">
                <label className="text-[10px] font-black uppercase tracking-widest text-[#94a3b8] block">Zawartość Dokumentu JSON</label>
                <textarea
                  value={bulkImportJson}
                  onChange={(e) => setBulkImportJson(e.target.value)}
                  placeholder="Wklej tablicę JSON tutaj..."
                  className="w-full h-56 bg-zinc-950 border border-zinc-900 rounded-xl px-4 py-3 text-white font-mono text-[11px] placeholder-zinc-700 focus:outline-none focus:border-orange-500 transition-all leading-normal"
                ></textarea>
              </div>

              {bulkImportError && (
                <div className="bg-red-500/10 border border-red-500/20 p-3.5 rounded-xl text-xs text-red-400 font-sans leading-relaxed">
                  <div className="font-extrabold uppercase tracking-wider text-[10px] text-red-500 mb-0.5">Błąd weryfikacji paczki</div>
                  {bulkImportError}
                </div>
              )}

              {bulkImportSuccess && (
                <div className="bg-emerald-500/10 border border-emerald-500/20 p-3.5 rounded-xl text-xs text-emerald-400 font-sans leading-relaxed">
                  <div className="font-extrabold uppercase tracking-wider text-[10px] text-emerald-500 mb-0.5">Sukces operacji</div>
                  {bulkImportSuccess}
                </div>
              )}
            </div>

            <div className="flex gap-3 justify-end border-t border-gray-900 pt-3">
              <button 
                onClick={() => setShowBulkImportModal(false)}
                className="bg-zinc-900 hover:bg-zinc-800 text-xs font-bold text-gray-400 hover:text-white px-5 py-2.5 rounded-xl transition cursor-pointer"
              >
                Anuluj
              </button>
              <button 
                onClick={handleBulkImportTracks}
                disabled={isImporting || !bulkImportJson.trim()}
                className={`bg-orange-600 hover:bg-orange-700 text-xs font-bold text-white px-6 py-2.5 rounded-xl transition cursor-pointer flex items-center gap-1.5 ${
                  (isImporting || !bulkImportJson.trim()) ? 'opacity-50 cursor-not-allowed' : ''
                }`}
              >
                {isImporting ? (
                  <>
                  ...
                  <>
                    <Database className="h-4 w-4" />
                    <span>Zatwierdź i Wgraj paczkę</span>
                  </>
                )}
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <div className="bg-[#121318] border border-gray-800 rounded-2xl p-6 w-full max-w-xl max-h-[90vh] overflow-y-auto space-y-4 shadow-2xl">
            ...
            <form onSubmit={handleSaveTrack} className="space-y-3.5 text-xs text-gray-300">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-[10px] font-bold uppercase tracking-wider text-gray-400">Oznaczenie Praw Autorskich (OZZ Exempt)</label>
                  <input 
                    type="text"
                    value={trackForm.copyrightLabel || ''} 
                    onChange={e => setTrackForm(prev => ({...prev, copyrightLabel: e.target.value}))}
                    className="w-full bg-black border border-gray-800 rounded-xl px-3 py-2 text-white focus:outline-none focus:border-red-500 text-[11px] font-mono"
                    placeholder="np. exempt-drm-hrl-direct"
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-bold uppercase tracking-wider text-gray-400">Warunki Licencyjne (License Terms)</label>
                  <input 
                    type="text"
                    value={trackForm.licenseTerms || ''} 
                    onChange={e => setTrackForm(prev => ({...prev, licenseTerms: e.target.value}))}
                    className="w-full bg-black border border-gray-800 rounded-xl px-3 py-2 text-white focus:outline-none focus:border-red-500 text-[11px] font-mono"
                    placeholder="np. HRL-direct-2026-unlimited"
                  />
                </div>
              </div>
