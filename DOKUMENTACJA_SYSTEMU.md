...
* **Autoryzacja:** Wymagany nagłówek `Authorization: Bearer <JWT>`.
* **Odpowiedź (200 OK):**
  ```json
  [
    {
...
* **Uwierzytelnienie:** Nagłówek `Authorization: Bearer <JWT>`.
...
* **Rozwiązanie operacyjne:** Pobierz kod z zakładki *Headless* w Admin Dashboardzie. Kod ten wykorzystuje domyślne filtry wtyczki Paid Memberships Pro (`pmpro_after_checkout`, `pmpro_after_change_membership_level`). Skopiuj go i wklej do pliku motywu `functions.php` lub do końca pliku `wp-config.php`. Kod ten przy rejestracji lub zmianie poziomu użytkownika w WordPress automatycznie wyśle asynchroniczne zapytanie typu POST na endpoint `/api/users` systemu HRL wraz z nagłówkiem `X-HRL-HEADLESS-SECRET`, tworząc lustrzany profil abonencki o tożsamym stopniu PMPro w pamięci operacyjnej RAM.
