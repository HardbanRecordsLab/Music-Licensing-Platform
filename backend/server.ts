...
import { attachUserOptional, requireAuth, requireRole } from "./src/middleware/auth.js";
import { createUser, getAllUsers, findUserById, findUserByEmail, verifyPassword, updateUser, deleteUser } from "./src/users.js";
import { generateToken } from "./src/auth.js";
import { generateAudioToken, verifyAudioToken, normalizeExpires } from "./src/audio-signing.js";


const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

// Sekrety i konfiguracja środowiskowa — walidowane w `src/env.ts`
const HMAC_SECRET = env.HMAC_SECRET;
const JWT_SECRET = env.JWT_SECRET;
// Globalny TTL dla tokenów dostępu (można nadpisać przez env TOKEN_TTL_SECONDS)
let TOKEN_TTL_SECONDS = env.TOKEN_TTL_SECONDS;

// Regex walidacji email (RFC 5322 simplified)
...
// Users are managed in src/users.ts and seeded at startup
...
const generateInitialPlayLogs = () => {
  ...
  for (let i = currentDays; i >= 0; i--) {
    ...
    for (let j = 0; j < playsToday; j++) {
      ...
      const subscribers = allUsers.filter(u => u.role === 'subscriber');
      const fallbackUser = { id: "u2", role: "subscriber" as const };
      const randomUser = subscribers.length > 0
...
async function startServer() {
  const app = express();
  const PORT = parseInt(process.env.PORT || "9108", 10);
  ...
  // Attach user from JWT if provided (no fallback to x-user-id or sim_user)

  app.use(attachUserOptional);

  const HEADLESS_SECRET = process.env.HEADLESS_SECRET || "";
  function requireHeadlessSecret(req: any, res: any, next: any) {
    const secretHeader = req.headers["x-hrl-headless-secret"];
    if (typeof secretHeader !== "string" || secretHeader !== HEADLESS_SECRET) {
      return res.status(401).json({ error: "Zły sekret headless lub brak nagłówka" });
    }
    next();
  }

  // Seed initial users (development). Passwords are plain here but hashed at runtime.
  await createUser("admin@hrl.pl", "adminpass", "HRL Administrator", "admin", 0, "u1");
  await createUser("aroma@b2b.pl", "subscriberpass", "Jan Kowalski (Aroma)", "subscriber", 1, "u2");
  await createUser("trendsetter@b2b.pl", "subscriberpass", "Marta Nowak (Trendsetter)", "subscriber", 2, "u3");
  await createUser("titan@b2b.pl", "subscriberpass", "Robert Lewandowski (Titan)", "subscriber", 3, "u4");
  await createUser("vip@b2b.pl", "subscriberpass", "Dyrektor Hotelu VIP", "subscriber", 4, "u5");

  generateInitialPlayLogs();

  // Health check for smoke tests and deployment monitoring
  app.get('/api/health', (req, res) => {
    res.json({ status: 'ok', uptime: process.uptime(), env: env.NODE_ENV, hostname: req.hostname });
  });
  ...
  app.get("/api/my-playlists", requireAuth, (req, res) => {
...
<body class="text-zinc-100 min-h-screen flex flex-col justify-between overflow-x-hidden p-6 md:p-12 bg-zinc-950">
  ...
  app.get("/api/playlist/:id", requireAuth, (req, res) => {
    ...
    const animatedTracksObj = playlistTracks.map(track => {
      const tokenSymbol = generateAudioToken({ filename: track.filename, userId: user.id, expiresAt });
      ...
      return {
        ...track,
        src: `/api/audio/${track.filename}?hrl_token=${tokenSymbol}&uid=${user.id}&expires=${Math.floor(new Date(expiresAt).getTime() / 1000)}`
      };
    });
  ...
  app.post("/api/token", requireAuth, (req, res) => {
    ...
    if (!filename) {
      return res.status(400).json({ error: "Brak parametru filename" });
    }

    const expiresAt = new Date(Date.now() + TOKEN_TTL_SECONDS * 1000).toISOString();
    const tokenSymbol = generateAudioToken({ filename, userId: user.id, expiresAt });
    const ip = req.ip || "127.0.0.1";
    const ipHash = crypto.createHash('sha1').update(ip).digest('hex');
    ...
    res.json({
      token: tokenSymbol,
      expiresAt,
      src: `/api/audio/${filename}?hrl_token=${tokenSymbol}&uid=${user.id}&expires=${Math.floor(new Date(expiresAt).getTime() / 1000)}`
    });
  });
  ...
  app.post("/api/play", requireAuth, (req, res) => {
    const user = (req as any).user as UserProfile | undefined;
    const { trackId, playlistId, durationS } = req.body;
  ...
  app.get("/api/stats/summary", requireAuth, requireRole('admin'), (req, res) => {
    const user = (req as any).user as UserProfile;
    if (!user) {
    ...
    res.json({
      ...
      revenueMonthly: getAllUsers().filter(u => u.role !== 'admin').reduce((sum, u) => {
        let price = 0;
  ...
  app.post("/api/users", async (req, res) => {
    ...
    const secretHeader = req.headers["x-hrl-headless-secret"];
    const headlessAuthorized = typeof secretHeader === "string" && secretHeader === HEADLESS_SECRET;

    if (!headlessAuthorized && (!adminUser || adminUser.role !== 'admin')) {
      return res.status(403).json({ error: "Brak dostępu" });
    }
    ...
    // Create persisted user (password generated if missing)
    const password = req.body.password || Math.random().toString(36).slice(2,10);
    const created = await createUser(newUser.email, password, newUser.name, newUser.role as any, newUser.pmproLevel, newUser.id);
    invoices.push(newInvoice);
    res.json({ user: created, invoice: newInvoice });
  });
  ...
  app.put("/api/users/:id", (req, res) => {
    ...
    const { id } = req.params;
    const updated = updateUser(id, req.body as any);
    if (!updated) return res.status(404).json({ error: "Nie zanaleziono użytkownika" });
  ...
  app.get("/api/audio/:filename", (req, res) => {
    ...
    const rawExpires = req.query.expires as string;

    if (!tokenSymbol || !userId || !rawExpires) {
      return res.status(403).send("Brak tokenu HMAC, identyfikatora usera lub parametru expires (Błąd 403).");
    }

    const expiresAt = normalizeExpires(rawExpires);
    if (!expiresAt) {
      return res.status(403).send("Nieprawidłowy parametr expires.");
    }

    const matchedToken = tokens.find(t => t.token === tokenSymbol && t.userId === userId && t.filename === filename && t.expiresAt === expiresAt);
    if (!matchedToken) {
      return res.status(403).send("Błędny token HMAC dostępu do pliku audio.");
    }

    if (!verifyAudioToken(tokenSymbol, filename, userId, rawExpires)) {
      return res.status(403).send("Token HMAC nie przeszedł weryfikacji lub wygasł.");
    }
  ...
  wss.on("connection", (ws, req) => {
    ...
    ws.on("close", () => {
      // Prawidłowe czyszczenie referencji WebSocket - zapobiega memory leak
      const index = activeWsClients.findIndex(c => c.id === connectionId);
      if (index !== -1) {
        const removed = activeWsClients.splice(index, 1)[0];
        // Wyczyść referencje, aby GC mógł zwolnić pamięć
        try {
          if (removed.ws && removed.ws.readyState !== WebSocket.CLOSED) {
            removed.ws.terminate();
          }
        } catch (e) {
          // WebSocket już zamknięty
        }
        // Jawne usunięcie referencji
        (removed as any).ws = null;
      }
      
      console.log(`[WS] Połączenie zamknięte: ${connectionId}. Aktywni klienci: ${activeWsClients.length}`);
  ...
  server.on("error", (err: any) => {
    if (err.code === "EADDRINUSE") {
      console.error(`Błąd: port ${PORT} jest już zajęty. Ustaw inny PORT w .env.local lub zakończ proces używający tego portu.`);
      process.exit(1);
    }
