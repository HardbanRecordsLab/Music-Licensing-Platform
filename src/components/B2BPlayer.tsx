...
export function B2BPlayer({ currentUserId, playlists: initialPlaylists }: B2BPlayerProps) {
                  ...
                  <div className="text-center md:text-left space-y-0.5">
                    {isCtaSpeaking ? (
                      ...
                      <>
                        ...
                        {/* 🛡️ REAL-TIME COPYRIGHT EXEMPTION CERTIFICATION STATUS INDICATOR */}
                        <div className="mt-2.5 p-2 bg-[#122220]/40 border border-emerald-500/20 rounded-xl flex items-start gap-2 text-left shadow-sm select-text">
                          <div className={`w-2 h-2 rounded-full mt-1 flex-shrink-0 ${isPlaying ? 'bg-emerald-400 animate-pulse' : 'bg-amber-400'}`} />
                          <div className="space-y-0.5 min-w-0 flex-1">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              <span className="text-[9px] font-black tracking-widest text-[#10b981] font-mono leading-none">
                                CERTYFIKAT EXEMPT-DRM
                              </span>
                              <span className="text-[7.5px] font-mono bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 px-1 py-0.2 rounded font-black uppercase">
                                ZWOLNIONY Z OZZ
                              </span>
                            </div>
                            <p className="text-[10px] text-[#f1f5f9] font-bold font-mono tracking-tight leading-normal truncate">
                              ID: HRL-CERT-{(activeTrack ? activeTrack.id : "PENDING").replaceAll("_", "-").toUpperCase()}-2026
                            </p>
                            <p className="text-[8.5px] text-zinc-400 font-medium font-sans leading-none">
                              Status: <span className="text-emerald-400 font-extrabold">{isPlaying ? 'ODTWARZANIE AUTORYZOWANE (ACTIVE)' : 'STAN NOMINALNY (IDLE)'}</span> • Bez opłat ZAIKS/ZPAV/STOART
                            </p>
                          </div>
                        </div>
                      </>
            ...
            <div className="space-y-5 max-w-xl text-center md:text-left flex-1 text-left">
              {isCtaSpeaking ? (
                ...
                <>
                  ...
                  {activeTrack?.isrc && (
                    <div className="flex flex-wrap gap-4 text-xs font-mono text-[#f1f5f9]0 font-semibold pt-2 justify-center md:justify-start items-center">
                      <span>KOD ISRC: {activeTrack.isrc}</span>
                      <span>TEMPO: {activeTrack.bpm} BPM</span>
                      <span className="flex items-center gap-1 bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded text-[10px] text-emerald-400 font-bold uppercase select-text tracking-normal">
                        <CheckCircle className="h-3.5 w-3.5 text-emerald-400" />
                        OZZ EXEMPT (CERT: HRL-CERT-{(activeTrack ? activeTrack.id : "PENDING").replaceAll("_", "-").toUpperCase()}-2026)
                      </span>
                    </div>
                  )}
                </>
